...
const inputCls =
  "rounded-xl border border-line bg-canvas text-fg placeholder:text-fg-subtle px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-ring";
const rowCls =
  "flex items-center gap-3 bg-surface border border-line rounded-2xl px-4 py-3 shadow-sm";
...
export default function CategoriesAdminPage() {
  ...
  const [editingBarrio, setEditingBarrio] = useState<{
    name: string;
    delivery_fee: string;
  }>({ name: "", delivery_fee: "" });
  ...
  async function handleAdd() {
    ...
    try {
      ...
      const { category } = await api.post<{ category: Category }>(
        "/api/categories",
        { name: newName.trim(), sort_order: nextOrder },
      );
  ...
  async function handleAddBarrio() {
    ...
    try {
      const nextOrder =
        barrios.length > 0
          ? Math.max(...barrios.map((b) => b.sort_order)) + 1
          : 1;
  ...
  async function handleSaveBarrio(b: Barrio) {
    ...
    try {
      const { barrio } = await api.put<{ barrio: Barrio }>(
        `/api/barrios/${b.id}`,
        {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-surface border border-line rounded-3xl p-6 shadow-sm">
          ...
          <p className="text-sm text-fg-muted mb-6">
            Gestiona las categorías de productos. El orden aquí se refleja en el
            menú del cliente.
          </p>
                ...
                <li key={cat.id} className={rowCls}>
                  <div className="flex flex-col gap-0.5">
                    <button
                      ...
                      disabled={idx === 0}
                      className={`${iconBtnCls} text-fg-subtle hover:text-fg disabled:opacity-20`}
                      aria-label="Subir"
                    >
                      <GripVertical
                        size={14}
                    ...
                    {editingId === cat.id ? (
                      <>
                        <button
                          ...
                          className={`${iconBtnCls} text-green-500 hover:bg-green-50 dark:hover:bg-green-950/40`}
                        >
                          <Check size={15} />
                        </button>
                        <button
                          onClick={() => setEditingId(null)}
                          className={`${iconBtnCls} text-fg-subtle hover:bg-surface-muted`}
                        >
                          <X size={15} />
                        </button>
                      </>
                      ...
                      <>
                        <button
                          ...
                          className={`${iconBtnCls} text-fg-subtle hover:text-brand hover:bg-brand/10`}
                        >
                          <Pencil size={15} />
                        </button>
                        <button
                          onClick={() => handleDelete(cat)}
                          className={`${iconBtnCls} text-fg-subtle hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40`}
                        >
                          <Trash2 size={15} />
                        </button>
                      </>
        ...
        <div className="bg-surface border border-line rounded-3xl p-6 shadow-sm">
          ...
          {barrioLoading ? (
            ...
            <p className="text-fg-subtle text-sm">
              No hay barrios. Crea el primero.
            </p>
          ) : (
            <ul className="space-y-2">
              {barrios.map((b) => (
                  ...
                  {editingBarrioId === b.id ? (
                    <>
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") handleSaveBarrio(b);
                          if (e.key === "Escape") setEditingBarrioId(null);
                        }}
                        className={`flex-1 ${inputCls} py-1`}
                      />
                      <input
                      ...
                      <>
                        <button
                          onClick={() => {
                            ...
                            setEditingBarrio({
                              name: b.name,
                              delivery_fee: String(b.delivery_fee),
                            });
                          }}
                          className={`${iconBtnCls} text-fg-subtle hover:text-brand hover:bg-brand/10`}
                        >
